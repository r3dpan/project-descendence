...
| Browsers | Server-set session cookie, from an OIDC login against an external IdP (Phase 9) | `HttpOnly`, `Secure`, `SameSite=Lax`; only a hash in `sessions.token_hash` |

Phase 7 built password auth first (decision #29) on the stated assumption that
OIDC would plug into the same `sessions` table and `RequireAuth` cookie path later,
changing only how the cookie gets issued. Phase 9 (decision #29 superseded, §6)
cashed that assumption in: local password auth is gone outright, not layered
alongside OIDC. `principals.password_hash` and its CHECK are dropped (migration
`00013_oidc.sql`); a `kind='user'` row instead carries nullable
`oidc_issuer`/`oidc_subject` with a `UNIQUE` pair and a one-directional CHECK
(`oidc_subject` may only be set on a `kind='user'` row, but a `kind='user'` row is
not required to carry one — a legacy/revoked principal kept only so
`runs.principal_id` keeps resolving has no OIDC identity and never will).

The flow (`internal/oidc`, `internal/api/session.go`):

1. `GET /api/v1/auth/login` — a browser navigation target, not a JSON endpoint
   (an XHR cannot follow a redirect to a third-party origin). Sets 5-minute
   `SameSite=Lax` state/nonce/PKCE-verifier cookies, then 302s to the IdP's
   authorization endpoint. Lax, not Strict — the IdP returns via a top-level
   navigation, and a Strict cookie would not be sent on that first request.
2. `GET /api/v1/auth/callback` — verifies `state`, exchanges the authorization
   code with PKCE, verifies the ID token's signature (via `internal/oidc`'s
   discovery-built verifier) and its `nonce` claim (go-oidc parses `nonce` onto
   `IDToken.Nonce` but does not itself verify it — that is this handler's job),
   then resolves `(iss, sub)`:
   - **Known, non-revoked** → mints a session, same as password login always did.
   - **Known, revoked** → refused (`403`), never resurrected.
   - **Unknown** → JIT-provisioned as a brand new `kind='user'` principal with
     **no role assigned**. `name` comes from `preferred_username` (falling back to
     `sub`, retried once with a short deterministic suffix on a `principals.name`
     collision), and is never refreshed on later logins.
3. `OIDC_BOOTSTRAP_USERNAME` — if set and a JIT-provisioned principal's
   `preferred_username` matches it, that principal gets the `admin` role in the
   same transaction as the insert. Creation-time only; a later login never
   re-checks it.

**No group claim is ever read.** Roles stay entirely local, in `principal_roles` —
this is deliberate and is what keeps the integration provider-agnostic: swapping
IdPs never requires mapping group names to roles, and role assignment is always a
separate, explicit, local admin action (`PATCH /api/v1/users/{id}`). A
roleless JIT principal has zero permissions; `RequirePermission` denies it
everything until an admin assigns a role, and the web UI renders one explanatory
screen for that state (`App.tsx`'s `Protected`) instead of a wall of 403s.

`POST /api/v1/auth/logout` still deletes the session row outright, not a
soft-delete — a dead session has no history worth keeping, unlike a run or a job.
`cmd/seed` lost `-kind user` entirely: there is nothing left for it to seed on the
browser-login path (the first admin now comes from `OIDC_BOOTSTRAP_USERNAME`), so
its token path is the sole recovery route left if the IdP is down.
...
| 29 | **SUPERSEDED by #34.** Phase 7's browser auth was a local username+password principal against a new `sessions` table, not OIDC | ARCHITECTURE.md §4.10 always allowed "a login form against a local account is fine first," and §7 kept deferring OIDC/Authentik past that phase regardless of which comes first - so building password auth then did not foreclose OIDC later, it just meant `RequireAuth`'s cookie path and the `sessions` table already existed for OIDC to plug into rather than being built alongside it. `password_hash` on `principals` mirrored `token_hash`'s pattern exactly (symmetric kind-tied CHECKs, hash-only storage), so a `user` principal and a `token` principal stayed visibly the same *kind* of thing rather than diverging in shape. Migration 00001's original comment ("kind='user' rows are placeholders for OIDC-backed browser sessions") was superseded by this, not merely stale - fixed in the same change (`00008_web_auth.sql`) per CLAUDE.md's doc rule | Superseded when Phase 9 actually built OIDC (decision #34) - the prediction that only cookie *issuance* would change, not the cookie path itself, held exactly as stated |
| 34 | **Phase 9's OIDC integration reads no group claim; roles stay entirely local in `principal_roles`, and local password auth is removed outright rather than kept as a fallback** | Reading a group claim and mapping it onto roles would tie this deployment's authorization model to one specific IdP's group-naming conventions, defeating the point of an OIDC integration meant to work with "any" provider (PLAN.md's Phase 9 goal). Keeping password auth as a fallback was rejected too: it would mean two live authentication paths to keep secure and tested indefinitely, for a benefit ("IdP is down") that `cmd/seed`'s token path already covers for the one principal that actually needs emergency access. A JIT-provisioned principal (unknown `(iss, sub)` on first login) is created with **no role** rather than a default one - `viewer` was considered and rejected, since a self-service default grant is a standing security decision no one actually made, versus an explicit admin action (`PATCH /api/v1/users/{id}`) an audit trail can point to. `OIDC_BOOTSTRAP_USERNAME` exists only to break the same chicken-and-egg problem `cmd/seed` already solves for tokens: the very first admin can't grant themselves `users:write` through an API gated on `users:write`, so a creation-time-only username match is the one first-admin exception. `principals.oidc_issuer`/`oidc_subject` got a one-directional CHECK (`kind='user'` required, but not required *of* every `kind='user'` row) rather than `password_hash`'s fully symmetric one, because a real legacy row (`webui-716`, task 9.1's audit) needed to survive the cutover soft-revoked, with no OIDC identity, purely so `runs.principal_id` kept resolving | A second IdP or a multi-tenant deployment needs group-claim-based role mapping - at homelab single-operator scale this has not come up and is not expected to |
...
| 31 | **A new Configuration web-UI page edits `DATABASE_URL`/`PODMAN_SOCKET` at runtime by writing a dedicated `config.env`-style file (`internal/appconfig`, default `$HOME/.config/descendence/config.env`, override via `DESCENDENCE_CONFIG_FILE`), not by editing the existing `.env` and not by moving either value into a Postgres `settings` table** | `DATABASE_URL` cannot be stored inside the database it connects to — a genuine bootstrap problem, not a style preference — so any Postgres-backed "settings" table was a non-starter for at least this one field, and keeping `PODMAN_SOCKET` in the same file rather than splitting the two across different storage was chosen for consistency over cleverness. A dedicated file rather than editing `.env` in place avoids the API ever parsing and rewriting a human-maintained file it doesn't fully own (comments, unrelated vars, ordering) — `RUN_LOG_DIR`/`GIT_REPO_DIR`/etc. stay exactly where they were, untouched, no scope creep. Both `cmd/api` and `cmd/supervisor` load the file at startup and layer it *under* an actual environment variable of the same name (`appconfig.Resolve`), so an ops/systemd `EnvironmentFile=` override still wins unconditionally — the web UI is additive, never able to silently override an intentional deployment-level pin. Neither process hot-reloads the file: a save just persists it, and the Configuration page tells the admin to restart both, matching this project's existing "config is read once at startup" posture everywhere else rather than adding a SIGHUP/reload path for one feature. The stored password is masked (`***`) on every `GET`, and `PutConfigHandler` splices the real password back in if a client resubmits that exact mask unchanged (only a real edit to the field persists a new password) — otherwise a "change only the socket path" save would silently corrupt the stored connection string | Multiple installations/profiles need more than two fields to be UI-configurable — at which point a small config *schema* (versioned keys, not just two hardcoded fields) is worth building, but two fields did not justify one yet |
| 32 | **Supervisor liveness for the Dashboard's status tile is a new `supervisor_heartbeat` singleton table, upserted every 5s by whichever supervisor holds the advisory lock, read by the API as stale after 15s (3×)** | The existing advisory lock (`cmd/supervisor/lock.go`, decision #6's process split) proves "am I the only supervisor," not "is a supervisor alive" — it's held on a dedicated connection and not introspectable for "who holds it" from the API's own connections. A heartbeat row is the same shape as every other api/supervisor coordination primitive in this codebase (§3: only through Postgres, never directly), so it extends an existing pattern rather than adding a new one. Beating on `pollInterval` (1s) was rejected as needless Postgres write load for a signal nobody needs sub-5-second resolution on; `scheduleSyncInterval`'s 5s cadence (already the codebase's precedent for "low-urgency periodic background work") was reused for consistency. The 3× staleness multiple absorbs one missed tick from transient DB latency without flapping the tile between up/down on every hiccup — the same "generous multiple over the write cadence" reasoning decision #18's retention sweep already uses. `SystemStatusHandler` always returns `200` (unlike `/healthz`'s `503`-on-unhealthy) because it exists to drive per-tile UI color, not to signal liveness to an infra prober — a `5xx` here would just fail the browser's fetch instead of rendering a red dot | Real run concurrency or multi-supervisor work ever arrives (already deferred, §7) and "a supervisor" stops meaning exactly one process — the heartbeat's singleton-row shape would need to become per-instance |
| 33 | **Users/Tokens/Configuration/Account are Settings sub-tabs (`/settings/account`, `/settings/users`, `/settings/tokens`, `/settings/configuration`) rendered by one `Settings.tsx` Tabs shell, not a nested-route/`<Outlet>` sidebar** | All four are "administer this instance" concerns that used to be unrelated top-level nav entries; grouping them removes three sidebar items without inventing new machinery — this codebase's SPA had zero nested-route/`<Outlet>` precedent anywhere, so a Tabs shell (Mantine's `Tabs`, first use here) reusing each page's existing component as a `Tabs.Panel` cost far less than building sidebar nesting for four pages. `activeTab` is derived from `location.pathname`, not internal component state, specifically so a deep link or a redirect lands on the right tab without a click ever having happened. The old `/users`/`/tokens` paths redirect (`<Navigate>`) rather than 404 or silently repoint to something else — cheap insurance for a real bookmarked URL in a single-user deployment, not defensive engineering for traffic that doesn't exist. `/settings/users/:id` stays a full separate route outside the Tabs shell (not a fifth tab) because a detail page has no sibling to switch between — it would be a "tab" with nothing else in its group | A fifth or sixth Settings concern arrives and four flat tabs stop being legible — nested grouping (tabs of tabs, or a sidebar-within-settings) would need to replace the flat `Tabs.List` at that point |
...
- Form builder (7.8)
- Per-schedule service tokens (split out of Phase 9's scope)
- A web setup wizard for first-run OIDC configuration (split out of Phase 9's scope)
